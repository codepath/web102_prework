...
function addGamesToPage(games) {
    ...
    for(let game of games){ //GL
        ...
        const gameInfo = `<img src= "${game.img}" class = "game-img">
                        <h2>${game.name}</h2>
...
let displayStr = `A total of $${totalRaised.toLocaleString('en-US')} has been raised for ${numGames} games. 
                  ${unfundedGames.length > 0 ? `Currently, ${unfundedGames.length} reamins unfunded. 
                  We need your help to fund these amazing games!` : "All games are funded. Thank you for your continued support"}`;

// create a new DOM element containing the template string and append it to the description container
let newParagraph = document.createElement("p");
newParagraph.innerHTML = displayStr;
document.querySelector("#description-container").append(newParagraph);
/************************************************************************************
...
console.log("firsGame: ", firstGame);
console.log("secondGame: ",secondGame);


// create a new element to hold the name of the top pledge game, then append it to the correct element
let topPledge = firstGame.name;
document.querySelector("#first-game").append(topPledge);
// do the same for the runner up item
let runnerUp = secondGame.name;
document.querySelector("#second-game").append(runnerUp);